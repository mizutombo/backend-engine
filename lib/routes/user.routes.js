...
userRouter
    ...
    .delete('/:username', ensureAuth, bodyParser, (req, res, next) => {
        const username = req.params.username;
        User.findOneAndRemove(username, (err) => {
            if (err) res.send(err, 'There was an error in deleting your account');
            else res.send('Your user account has been deleted!');
        });
    });
